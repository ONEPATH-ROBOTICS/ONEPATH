...
It uses the ONEPATH API to send movement commands based on key presses.
...
ONEPATH_API_KEY = os.environ.get("ONEPATH_API_KEY", None)
if not ONEPATH_API_KEY:
    logging.error("ONEPATH_API_KEY environment variable not set.")
    sys.exit(1)
...
class RemoteMoveController:
    def __init__(self):
        self.ws_client = ws.Client(
            url=f"wss://one-path.io/api/core/teleops/command?api_key={ONEPATH_API_KEY}"
        )
