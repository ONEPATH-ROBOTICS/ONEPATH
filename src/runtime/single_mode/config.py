...
def load_config(
    ...
    if g_api_key is None or g_api_key == "" or g_api_key == "onepath_free":
        logging.warning(
            "No API key found in the configuration file. Checking for backup ONEPATH_API_KEY in your .env file."
        )
        backup_key = os.environ.get("ONEPATH_API_KEY")
        g_api_key = backup_key
        if backup_key:
            raw_config["api_key"] = backup_key
            logging.info("Success - Found ONEPATH_API_KEY in your .env file.")
        else:
            logging.warning(
                "Could not find any API keys. Please get a free key at one-path.io."
            )
