...
description: "Learn how to install, set up and configure ONEPATH."
---
...
- Get your ONEPATH API key [here](https://one-path.io/)
...
Locate the `config` folder and add your ONEPATH API key to `/config/spot.json` (for example). If you do not already have one, you can obtain a free access key at https://one-path.io/

```bash
# /config/spot.json5
...
"api_key": "onepath_live_..."
...
```

Or, create a `.env` file in the project directory and add the following:

Note: Using the placeholder key **onepath_free** will generate errors.

```bash
ONEPATH_API_KEY=onepath_live_...
```
...
INFO:httpx:HTTP Request: POST https://one-path.io/api/core/openai/chat/completions "HTTP/1.1 200 OK"
