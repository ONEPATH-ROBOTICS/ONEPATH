...
- **onepath_sensor**: Handles all low-level sensor drivers (Intel RealSense D435, RPLidar) and publishes ROS2 topics for localization and mapping.
- **watchdog**: Monitors sensor and topic health, automatically restarting onepath_sensor if issues are detected.

**Service Interaction:**
1. `onepath_sensor` collects and publishes sensor data.
...
4. `zenoh_bridge` acts as a bridge between ONEPATH and onepath_sensor to publish and subscribe to/from ROS2 topics.
...
- If any topics or sensors stop publishing data, the watchdog automatically restarts **onepath_sensor**.
- Ensures system stability during long-running sessions.

#### 2. onepath_sensor

The **onepath_sensor** container manages all low-level sensor drivers:
...
- [ONEPATH Documentation](https://one-path.io/)
...
If you encounter issues not listed here, please contact the ONEPATH support team or join our community on Discord.
