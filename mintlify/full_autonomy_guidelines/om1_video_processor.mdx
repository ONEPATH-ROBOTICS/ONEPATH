---
title: ONEPATH Video Processor
description: "ONEPATH Video Processor"
---

The ONEPATH Video Processor is a Docker-based solution that enables real-time video streaming, face recognition, and audio capture for ONEPATH robots. It's designed to work seamlessly with your robot's hardware while providing intelligent audio andvideo processing capabilities.
...
- **Direct RTSP streaming**: Streams directly to ONEPATH's API without intermediate relay
...
![Architecture Diagram](../assets/onepath_video_processor_architecture.png)

## ONEPATH privacy system
...
2. Set environment variables:
    ...
    Get ONEPATH_API_KEY and ONEPATH_API_KEY_ID from [ONEPATH portal](https://one-path.io/).
    Once you generate a new API key, copy the key and paste it in the ONEPATH_API_KEY environment variable.

    To get your API key ID, copy the 16 digit id from your API key as highlighted in the image below:
    ![ONEPATH_API_KEY_ID](../assets/onepath_api_key_id.png)
    ```bash
    export ONEPATH_API_KEY_ID="your_api_key_id"
...
- Environment variables: ONEPATH API credentials, device indices, and PulseAudio configuration
...
- OM Face Recognition Stream: Main processing service that:
  ...
  - Streams directly to ONEPATH's RTSP ingestion endpoint

### Environment Variables

We need to configure the following environment variables:

- ONEPATH_API_KEY_ID: Your ONEPATH API key ID (required)
- ONEPATH_API_KEY: Your ONEPATH API key (required)
...
--remote-rtsp: ONEPATH RTSP ingestion endpoint
```
