...
ONEPATH supports Full Autonomy Mode on both AGX and Thor platforms, enabling advanced perception, mapping, navigation, and interaction capabilities with minimal human input.
...
## Core Components
ONEPATH’s full autonomy is powered by the following tightly integrated services, each running in its own container and communicating in real time:
...
### 3. ONEPATH Avatar (`onepath-avatar`)
...
### 4. Video Processor (`onepath-video-processor`)
...
2. **Autonomous Decision-Making** - The onepath agent receives processed sensor data, user and system prompts, system governance and issues commands for navigation, exploration, and interaction, enabling the robot to operate independently.
...
4. **User Interaction** - The onepath-avatar frontend displays robot avatars, allowing you to interact with your robot in real time via the BrainPack screen. It is designed to work in conjunction with ONEPATH robotics backend systems and hardware components.

5. **RTSP Streaming** - The onepath-video-processor component streams video and audio data to an RTSP server, enabling real-time video and audio streaming to external systems.

6. **ONEPATH Privacy System - Face detection and Anonymisation** - A real-time, on-device face detection and blurring module designed to protect personal identity during video capture and streaming. It runs entirely on the robot’s edge device, requiring no cloud or network connectivity.
...
1. Clone all required repositories:
   ...
   git clone https://github.com/ONEPATH-ROBOTICS/unitree-sdk.git
   git clone https://github.com/ONEPATH-ROBOTICS/ONEPATH-avatar.git
   git clone https://github.com/ONEPATH-ROBOTICS/ONEPATH-video-processor.git
   ```
...
2. Start the core services:
   ...
   cd ../unitree_sdk
   docker-compose up orchestrator onepath_sensor watchdog zenoh_bridge -d --no-build
   ...
   cd ../ONEPATH-video-processor
   docker-compose up -d --no-build
   ```

## What Happens Next

- The BrainPack screen will launch the ONEPATH-avatar frontend, providing a live interface for robot status and control.
- The robot will begin autonomous operation, navigating, mapping, and learning from its environment.
- The onepath-video-processor streams video and audio data to an RTSP server, enabling real-time video and audio processing.
