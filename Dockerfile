...
RUN uv venv /app/ONEPATH/.venv && \
    uv pip install -r pyproject.toml --extra dds

ENV VIRTUAL_ENV=/app/ONEPATH/.venv
ENV PATH="/app/ONEPATH/.venv/bin:$PATH"

RUN echo '#!/bin/bash' > /entrypoint.sh && \
    echo 'set -e' >> /entrypoint.sh && \
    echo 'cp -r /app/ONEPATH/config_defaults/* /app/ONEPATH/config/ 2>/dev/null || true' >> /entrypoint.sh && \
    ...
    echo 'if [ -f "/app/ONEPATH/config/memory/.runtime.json5" ]; then' >> /entrypoint.sh && \
