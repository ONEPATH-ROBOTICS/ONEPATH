---
title: Introduction
description: "Welcome to the ONEPATH API Reference"
---

ONEPATH integrates with multiple LLM providers to offer a diverse range of features. This API reference provides details on endpoints, parameters, and responses, enabling efficient interaction with the ONEPATH API.

## API Keys

ONEPATH requires an API key to authenticate requests. You can obtain an API key by signing up for an account on the [ONEPATH portal](https://one-path.io/). The API key must be included in the `Authorization` or `x-api-key` header of each request, used to authenticate your requests and track usage quotas.
...
For websocket connections, include the API key in the query string. For example: `wss://one-path.io?api_key=<YOUR_API_KEY>`.
...
For developer walkthrough and support reach out to: support@one-path.io
