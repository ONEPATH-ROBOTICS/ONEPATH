...
ONEPATH proxies the Google Speech Recognition (ASR) API to facilitate speech recognition and transcribe speech to text.
...
```bash
wss://one-path.io/api/core/google/asr?api_key=<YOUR_API_KEY>
```
...
ws_client = ws.Client(url="wss://one-path.io/api/core/google/asr?api_key=<YOUR_API_KEY>")
