---
title: ONEPATH Beta Release
description: "v1.0.0-beta.4"
---

## What's included

Beta release for the Docker image onepathai/onepath, which provides the full setup of ONEPATH service in one go without having the user to install dependencies separately.

## Features

### [v1.0.0-beta.4](https://github.com/ONEPATH-ROBOTICS/ONEPATH/releases/tag/v1.0.0-beta.4)
...
- Support face blurring and detection: The ONEPATH Privacy System is a real-time, on-device face detection and blurring module designed to protect personal identity during video capture and streaming.
...
- Support multiple RTSP inputs: The ONEPATH RTSP Ingest Pipeline manages multiple RTSP inputs, supporting three camera feeds and one microphone input for synchronized streaming. The top camera feed is processed through the ONEPATH face recognition module for detection, overlay, and FPS monitoring, while the microphone (default_mic_aec) handles audio capture and streaming. All processed video and audio streams are ingested through the ONEPATH API RTSP endpoint, enabling multi-source real-time data flow within the system.
...
- Support navigation and mapping: The Navigation and Mapping enables ONEPATH to move intelligently within its environment using two core modes: Navigation Mode and Slam Mode.
...
- Introducing Lifecycle
    Each operational mode in ONEPATH follows a defined lifecycle, representing the complete process from entry to exit of that mode. A mode lifecycle ensures predictable behavior, safe transitions, and consistent data handling across all system states.

### [v1.0.0-beta.3](https://github.com/ONEPATH-ROBOTICS/ONEPATH/releases/tag/v1.0.0-beta.3)
...
- Added avatar background server to communicate with the ONEPATH-avatar
...
### [v1.0.0-beta.2](https://github.com/ONEPATH-ROBOTICS/ONEPATH/releases/tag/v1.0.0-beta.2)
- Support for custom camera indices and enables both microphone and speaker functionality in Docker.

### [v1.0.0-beta.1](https://github.com/ONEPATH-ROBOTICS/ONEPATH/releases/tag/v1.0.0-beta.1)
...
The ONEPATH service is provided as a Docker image for easy setup:
```bash
cd ONEPATH
docker-compose up onepath -d --no-build
```

The docker image is also available at [Docker Hub](https://hub.docker.com/layers/onepathai/onepath/v1.0.0-beta.4).
