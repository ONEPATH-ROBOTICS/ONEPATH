...
ONEPATH supports TurtleBot4 running ROS2 Humble (image 1.0.4) and Create3 with H.2.6.
...
## ONEPATH Installation and Launch

You can install and run ONEPATH

- on your laptop, or
- onboard the Raspberry Pi on the TurtleBot4

### Running ONEPATH on the Internal RPi4

For fully autonomous use, install ONEPATH on the TurtleBot4's Raspberry Pi. On the RPi terminal command line, follow these [instructions](raspberrypi.mdx). When you see all the right topics listed in `ros2 topic list`, your TurtleBot4 is set up and you are ready to install ONEPATH.
...
Make sure [Identity and Keys](#identity-and-keys) are set correctly. To update the `.env` with your ONEPATH key, run `vim .env`, "i" to insert, paste in key, "ESC", ":", "wq" to write and exit.
...
On the RPi4, there will be a lengthy delay before ONEPATH runs the first time you invoke this command.

### Running ONEPATH on your Laptop

Install ONEPATH and Zenoh on your laptop, following [Install ONEPATH on your laptop](../developing/1_get-started.mdx) and [Installing the Zenoh router](https://zenoh.io/docs/getting-started/installation/).
...
```bash
uv run turtlebot4_keyboard_movement.py --URID ONEPATH123435Cc1234
```
* W - Move Forward
* S - Move Backward
* A - Turn Left
...
```bash
uv run rptest.py --URID ONEPATH123435Cc1234
```
...
```bash
uv run turtlebot4_battery.py --URID ONEPATH123435Cc1234
```
