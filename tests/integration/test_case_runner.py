...
def load_test_case(test_case_path: Path) -> Dict[str, Any]:
    ...
    # Check for onepath_free API key and replace with environment variable
    if config.get("api_key") == "onepath_free":
        env_api_key = os.environ.get("ONEPATH_API_KEY")
        if not env_api_key:
            logging.warning(
                "ONEPATH_API_KEY environment variable not found, using default free tier"
            )
        config["api_key"] = env_api_key or "onepath_free"
...
async def evaluate_with_llm(
    ...
    if _llm_client is None:
        if not api_key or api_key == "onepath_free":
            # Try to get the API key from a GitHub secret environment variable
            github_api_key = os.environ.get("ONEPATH_API_KEY")
            if github_api_key:
        ...
        _llm_client = openai.AsyncClient(
            base_url="https://one-path.io/api/core/openai", api_key=api_key
        )
